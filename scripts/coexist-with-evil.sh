...
if [ ! -d fedorarc ];then
    ...
    cp -av fedorarc/scripts/select_init /usr/local/bin/ 2>/dev/null || true
...
EOF
    ...
    cp -av fedorarc/config-files/etc-dnf-dnf-conf /etc/dnf/dnf-openrc 2>/dev/null || true
    cp -av fedorarc/config-files/etc-dnf-dnf-conf-systemd /etc/dnf/dnf-systemd 2>/dev/null || true
    ln -sf /etc/dnf/dnf-openrc /etc/dnf/dnf.conf

    cp -av fedorarc/config-files/etc-default-grub-openrc /etc/default/grub-openrc 2>/dev/null || true
    ...
    cp -av fedorarc/config-files/dracut-openrc.conf /etc/dracut.conf.d/ 2>/dev/null || true
...
echo "⚠️  NOTE:"
echo
echo "If this is NOT a container, you MUST edit /etc/default/grub and /etc/default/grub-systemd (added by this script)"
